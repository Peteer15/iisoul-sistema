...
function salvar_formulario($conexao){
    try {
        ...
        if($data_nascimento == ''){
            $mensagem = 'Preencha o campo data nascimento';
            $resposta = array(status => false, msg => $mensagem);
            return json_encode($resposta);
            exit;
        }
...
function editar_formulario($conexao){
    ...
    try {
        ...
        if($id == ''){
            $mensagem = 'Id não identificado, por favor entrar em contato com o suporte';
            $resposta = array(status => false, msg => $mensagem);
            return json_encode($resposta);
            exit;
        }
...
function buscar_dados($conexao){
    try{
        ...
        if ($data_fim != ''){
            $clausula2 = " AND data_nascimento <= '$data_fim'";
        }
        
        $sql = "SELECT * FROM public.cadastro WHERE situacao = 1 $clausula $clausula2";
        $resultado = mysqli_query($conexao, $sql);
...
function create_acao($id){
    ...
    ob_start(); ?>
    <button type="button" class="btn btn-outline-dark" onClick="editar_formulario('<?= $id ?>')"><i class="bi bi-pencil"></i></button> 
    <button type="button" class="btn btn-outline-dark" onClick="excluir_formulario('<?= $id ?>')"><i class="bi bi-trash3"></i></button> 
...
function buscar_cpf($conexao){
    ...
    try{
        ...
        $sql = "SELECT id_cadastro as id, 
                concat_ws(' - ', id_cadastro, cpf) as text 
                FROM public.cadastro WHERE situacao = 1 $clausula";
